    ...
    IOptions<PulseFlowDiagnosticsOptions> diagnosticsOptions) : BackgroundService
{
    ...
    private void NotifyUnmatched(IPulse pulse)
    {
        ...
        {
            Trace.TraceError(
                "[Novolis.Messaging] PulseFlowDiagnosticsOptions.UnmatchedPulse callback threw (pulseId={0}): {1}",
                pulse.Id,
                ex);
        }
    }
    ...
    private async Task InvokeFlowAsync(IFlow flow, IPulse pulse, CancellationToken stoppingToken)
    {
        ...
        {
            Trace.TraceError(
                "[Novolis.Messaging] Flow {0} failed while handling pulse {1} (pulseId={2}): {3}",
    ...
    private void NotifyFlowFault(IFlow flow, IPulse pulse, Exception exception)
    {
        ...
        {
            Trace.TraceError(
                "[Novolis.Messaging] PulseFlowDiagnosticsOptions.FlowFault callback threw (pulseId={0}): {1}",
                pulse.Id,
                ex);
        }
    }
